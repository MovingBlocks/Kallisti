...
public abstract class Machine {
    ...
    private void registerRules(Object parent, Method m) throws IllegalArgumentException {
        ...
        if (rule != null) {
            ...
            if (r.method.getParameterCount() == 0) {
                throw new IllegalArgumentException("Rule must accept a parameter! If you want to always add a " +
                        "component to a machine, do so directly.");
            }
    ...
    /**
     * Register the component rules present in static methods and constructors of the given class.
     *
    ...
    /**
     * Add a non-component object accessible to components during creation. In general, these should be objects related
     * to a given Machine, such as the Machine itself.
     *
    ...
    /**
     * Add a new component to the Machine. If a @ComponentInterface-marked class or subclass is already present with the
     * same context, the component will not be added.
     *
    ...
    public boolean addComponent(ComponentContext context, Object o) {
        ...
        for (Class c : KallistiReflect.classes(objectClass)) {
            if (isComponentInterface(c)) {
                Map<ComponentContext, ComponentEntry> m = entryClassContextTable.computeIfAbsent(c,
                        (cc -> new HashMap<>()));
     ...
     * @param c The class of the component. Should be a (sub)class or (sub)interface marked with
     *         @ComponentInterface.
